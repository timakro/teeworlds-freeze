...
void IGameController::Teamscore(int Team)
{
	if(Team == 0) {
		...
		if(Team == 0) {
			for(int i=0; i < Server()->MaxClients(); i++) {
				if(Server()->ClientIngame(i)) {
					char aBuf[128];
					str_format(aBuf, sizeof(aBuf), "Red team scored a point!");
			...
			for(int i=0; i < Server()->MaxClients(); i++) {
				if(Server()->ClientIngame(i)) {
					GameServer()->SendBroadcast("Blue team scored a point!", i);
				}
			}
		}

		for(int i=0; i < Server()->MaxClients(); i++) {
			if(Server()->ClientIngame(i) && GameServer()->m_apPlayers[i]->GetCharacter()) {
				GameServer()->m_apPlayers[i]->GetCharacter()->Die(GameServer()->m_apPlayers[i]->GetCID(),WEAPON_WORLD, false);
			}
		}
	}
}
...
void IGameController::StartRound()
{
	...
	JoinTime = true;
	for(int i=0; i < Server()->MaxClients(); i++) {
		if(Server()->ClientIngame(i) && GameServer()->m_apPlayers[i]->GetTeam() == TEAM_SPECTATORS) {
			GameServer()->SendBroadcast("You can now join a team", i);
		}
	}
}
...
void IGameController::Tick()
{
	...
	if(Server()->Tick() == m_RoundStartTick+Server()->TickSpeed()*10)
		JoinTime = false;

	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if (MsgID == NETMSGTYPE_CL_SETTEAM && !m_World.m_Paused)
		{
			...
			for(int i=0; i < Server()->MaxClients(); i++) {
				if(Server()->ClientIngame(i) && m_apPlayers[i]->GetTeam() != TEAM_SPECTATORS) {
					players++;
				}
			}
			if(m_pController->JoinTime || players < 2 || pMsg->m_Team == TEAM_SPECTATORS)
			{
				...
				SendBroadcast("You can only join a team when a new round starts", ClientID);
		}
